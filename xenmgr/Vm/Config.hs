...
getXlConfig cfg =
  ...
  where
    ...
    prelude = do Just uuid <- readConfigProperty uuid vmUuidP :: Rpc (Maybe Uuid)
                 ...
                 hd_type <- readConfigPropertyDef uuid vmHdType ""
...
diskSpec uuid d  = do
  ...
  return $ printf "'%s,%s,%s,%s,%s,%s'"
             (diskPath d) (fileToRaw (enumMarshall $ diskType d)) (cdType stubdom d) (disk_Device d hd_type) (enumMarshall $ diskMode d) (if ((enumMarshall $ diskDeviceType d) == "cdrom") then (enumMarshall $ diskDeviceType d) else "")
  where
    cdType stubdom d =
